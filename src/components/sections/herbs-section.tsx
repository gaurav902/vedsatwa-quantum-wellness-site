  ...
  return (
    <section className="py-20 px-4 bg-background">
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {herbs.map((herb, index) => (
            <div 
              key={index} 
              className="relative overflow-hidden rounded-2xl bg-gradient-to-b from-[#f6d4d2] to-black transition-transform duration-300 hover:scale-[1.02] hover:shadow-xl"
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-black/90 p-6 min-h-[200px]">
                ...
                <button 
                  className="absolute bottom-6 right-6 w-8 h-8 rounded-full bg-[#f6d4d2] flex items-center justify-center transform transition-transform hover:scale-110"
      ...
      <Dialog open={!!selectedHerb} onOpenChange={() => setSelectedHerb(null)}>
        <DialogContent className="glass max-w-2xl bg-black/80 border-[#f6d4d2]/30 text-white">
          {selectedHerb && (
            <div className="p-2">
              <DialogTitle className="text-2xl font-bold text-[#f6d4d2] text-center mb-4">
                {selectedHerb.name}
              </DialogTitle>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <ul className="ml-5 space-y-2">
                    {selectedHerb.benefits.map((benefit, idx) => (
                      <li key={idx} className="list-disc text-white/90">{benefit}</li>
                    ))}
                  </ul>
                </div>
                ...
                <div>
                  ...
                  <p className="text-white/90">{selectedHerb.usage}</p>
                </div>
